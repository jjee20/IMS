...
namespace PresentationLayer.Presenters
{
    public class ProjectPresenter
    {
        ...
        {
            var model = await _unitOfWork.Project.Value.GetAsync(c => c.ProjectId == _view.ProjectId, tracked: true);
            ...
            try
            {
                ...
                {
                    await _unitOfWork.Project.Value.AddAsync(model);
        ...
        private void Delete(object? sender, EventArgs e)
        {
            try
            {
                ...
                {
                    _view.IsSuccessful = false;
                    _view.Message = "Please select project(s) to delete.";
                    _view.ShowMessage(_view.Message);
                    return;
                }

                var selectedProjects = _view.DataGrid.SelectedItems.Cast<ProjectViewModel>().ToList();
                var ids = selectedProjects.Select(p => p.ProjectId).ToList();
                ...
                {
                    _view.IsSuccessful = false;
                    _view.Message = "Selected projects could not be found.";
                    _view.ShowMessage(_view.Message);
                    return;
                }
                ...
                _view.IsSuccessful = true;
                _view.Message = $"{entities.Count} project(s) deleted successfully.";
                _view.ShowMessage(_view.Message);
                LoadAllProjectList();
            }
            catch (Exception ex)
